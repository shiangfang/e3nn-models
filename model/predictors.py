...
def predict_energy_forces_stress(
    ...
    def energy_fn(positions, cell):
        ...
        node_energies = model(
            vectors, graph.nodes.species, graph.senders, graph.receivers, graph.nodes.efield,
        )  # [n_nodes, ]
...
def predict_becs_eps(
    model, graph: jraph.GraphsTuple
) -> Dict[str, jnp.ndarray]:
    """Predict becs, eps tensors
    """

    def model_fn(positions, cell):
        vectors = get_edge_relative_vectors(
            positions=positions,
            senders=graph.senders,
            receivers=graph.receivers,
    ...
    node_becs -= node_becs_avg_repeat
    return {
        "becs": node_becs,  # 
