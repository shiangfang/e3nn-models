...
class Configuration:
    ...
    efield: Optional[Efield] = None
    cell: Optional[Cell] = None
    pbc: Optional[Pbc] = None
    becs: Optional[np.ndarray] = None
...
def config_from_atoms(
    ...
    efield = atoms.info.get(efield_key, None)
...
def load_from_xyz(
    ...
    if extract_atomic_energies:
        ...
        #print(len(atoms_without_iso_atoms),atomic_energies_dict)
...
GraphNodes = namedtuple("Nodes", ["positions", "forces", "species","efield"])
...
class GraphDataLoader:
    def __init__(
        ...
        self._length = None
        self.rng = np.random.default_rng(loader_seed)
...
def config_from_atoms_becs(
    ...
    eps = atoms.info.get(eps_key, None)  
    #assert eps.shape == (3, 3)
    becs = atoms.arrays.get(becs_key, None)
    
    eps = eps.reshape((3,3))
    becs = becs.reshape((len(atoms),3,3))
    
    #print(eps.shape,becs.shape)
    
    #print('eps and becs',eps,becs)
...
GraphNodesBecs = namedtuple("Nodes", ["positions", "becs", "species"])
GraphGlobalsBecs = namedtuple("Globals", ["cell", "eps", "weight"])
    
def graph_from_configuration_becs(config: Configuration, cutoff: float) -> jraph.GraphsTuple:
    senders, receivers, shifts = get_neighborhood(
        positions=config.positions, cutoff=cutoff, pbc=config.pbc, cell=config.cell
    )
    return jraph.GraphsTuple(
        nodes=GraphNodesBecs(
            positions=config.positions,
            #forces=config.forces,
            becs = config.becs,
            species=config.atomic_numbers,
